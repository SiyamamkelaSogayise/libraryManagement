...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Function to populate the book table with filtered books
    function populateBookTable(filteredBooks) {
        ...
        filteredBooks.forEach(book => {
            ...
            row.innerHTML = `
                ...
                <td>${book.publicationDate}</td>
                <td>${book.category}</td>
                <td>${book.numberOfCopies}</td> <!-- Display number of copies -->
            `;
            bookTableBody.appendChild(row);
        });
    ...
    const availableBooks = [
        { title: 'Book 1', author: 'Author A', publicationDate: '2023-01-01', category: 'Fiction', numberOfCopies: 5 },
        { title: 'Book 2', author: 'Author B', publicationDate: '2022-05-15', category: 'Non-Fiction', numberOfCopies: 3 },
        { title: 'Book 3', author: 'Author C', publicationDate: '2021-12-31', category: 'Biography', numberOfCopies: 7 }
        // Add more books as needed
    ];

    // Initial population of table (example)
    populateBookTable(availableBooks);
    ...
    // Event listener for search form submission (optional)
    const searchBooksForm = document.getElementById('searchBooksForm');
    searchBooksForm.addEventListener('submit', function(e) {
        e.preventDefault();
        const searchTermInput = document.getElementById('searchTerm');
